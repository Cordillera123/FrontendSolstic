...
const getLucideIcon = (iconName) => {
  ...
  const variations = [
    ...
    iconName.replace(/[-\s]/g, "").charAt(0).toUpperCase() +
      iconName.replace(/[-\s]/g, "").slice(1), // Sin separadores + primera mayúscula
  ];
  ...
  const flexibleMatch = iconKeys.find(
    (key) =>
...
const IconSelector = ({
  ...
  if (icons.length > 0) {
    ...
    console.log(
      "🎯 IconSelector - Nombres de iconos disponibles:",
  ...
  const filteredIcons = useMemo(() => {
    return icons.filter((icon) => {
      ...
      const matchesCategory =
        !selectedCategory || iconCategory === selectedCategory;
  ...
  const categories = useMemo(() => {
    const cats = [
      ...new Set(
        icons.map((icon) => icon.ico_cat || icon.categoria).filter(Boolean)
      ),
    ];
    return cats.sort();
  }, [icons]);
  ...
  const selectedIconData = useMemo(() => {
    ...
    return icons.find((icon) => (icon.ico_id || icon.id) == selectedIcon);
  }, [selectedIcon, icons]);
  ...
  return (
    <div className="relative">
      {/* Botón principal del selector - ALTURA CORREGIDA */}
      <button
        type="button"
        onClick={toggleDropdown}
        disabled={disabled}
        className={`w-full flex items-center justify-between px-3 py-2 border rounded text-sm transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent h-[42px] ${
          disabled
        ...
        <div className="flex items-center flex-1 min-h-0 overflow-hidden">
          {selectedIconData ? (
            <div className="flex items-center">
              ...
              />
              <div className="text-left max-w-[110px] overflow-hidden">
            ...
            <div className="flex items-center">
              <div className="w-5 h-5 bg-gray-200 rounded mr-2 flex items-center justify-center flex-shrink-0">
              ...
              >
                ...
                {categories.map((category) => (
                  <option key={category} value={category}>
                    ...
                    {
                      icons.filter(
                        (i) => (i.ico_cat || i.categoria) === category
                      ).length
                    }
                    )
                  </option>
