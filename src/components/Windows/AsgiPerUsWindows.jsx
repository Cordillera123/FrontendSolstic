...
const PermissionTreeItem = memo(
  ({
    menu,
    hasUserPermission,
    isPermissionAvailableInProfile,
            ...
            <div className="flex items-center flex-1">
              ...
              <span
                className={`font-medium ${
                  menuAvailable ? "text-gray-900" : "text-red-700"
                }`}
              >
                {menu.men_nom}
              </span>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center text-xs text-gray-500">
              <Icon
                ...
                size={12}
                className={`mr-1 ${
                  menuAvailable ? "text-green-600" : "text-red-600"
                }`}
              />
            ...
            <label className="flex items-center cursor-pointer">
              ...
              <span
                className={`text-sm ${
                  !menuAvailable || savingPermissions
                    ? "text-gray-400"
                    : "text-gray-700"
                }`}
              >
...
const SubmenuTreeItem = memo(
  ({
    ...
    const submenuAvailable = isPermissionAvailableInProfile(
      menuId,
      submenu.sub_id
    );
            ...
            <div className="flex items-center flex-1">
              ...
              <span
                className={`text-sm font-medium ${
                  submenuAvailable ? "text-purple-900" : "text-red-700"
                }`}
              >
                {submenu.sub_nom}
              </span>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center text-xs text-gray-500">
              <Icon
                ...
                size={10}
                className={`mr-1 ${
                  submenuAvailable ? "text-green-600" : "text-red-600"
                }`}
              />
            ...
            <label className="flex items-center cursor-pointer">
              ...
              <span
                className={`text-xs ${
                  !submenuAvailable || savingPermissions
                    ? "text-gray-400"
                    : "text-gray-700"
                }`}
              >
        ...
        {hasOptions && isSubmenuExpanded && (
          <div className="ml-4 mt-2 space-y-1">
            {submenu.opciones.map((opcion) => {
              const opcionAvailable = isPermissionAvailableInProfile(
                menuId,
                submenu.sub_id,
                opcion.opc_id
              );
              const opcionGranted = hasUserPermission(
                menuId,
                submenu.sub_id,
                opcion.opc_id
              );
              ...
              return (
                ...
                >
                  <div className="flex items-center flex-1">
                    ...
                    <span
                      className={`text-sm ${
                        opcionAvailable ? "text-green-900" : "text-red-700"
                      }`}
                    >
                      {opcion.opc_nom}
                    </span>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center text-xs text-gray-500">
                      <Icon
                        ...
                        size={10}
                        className={`mr-1 ${
                          opcionAvailable ? "text-green-600" : "text-red-600"
                        }`}
                      />
                    ...
                    <label className="flex items-center cursor-pointer">
                      ...
                      <span
                        className={`text-xs ${
                          !opcionAvailable || savingPermissions
                            ? "text-gray-400"
                            : "text-gray-700"
                        }`}
                      >
...
const loadUserPermissions = useCallback(async (userId) => {
    ...
    try {
        const response = await fetch(`http://127.0.0.1:8000/api/usuarios/${userId}/permissions`, {
            headers: {
                Authorization: `Bearer ${localStorage.getItem("auth_token")}`,
                "Content-Type": "application/json",
            },
        ...
        const result = await response.json();
        if (result.status === "success") {
            setUserPermissions(result.data.permisos || []);
        }
  ...
  const toggleUserPermission = useCallback(
    async (menId, subId = null, opcId = null, currentState) => {
      if (!selectedUser) return;

      setSavingPermissions(true);
      try {
        const response = await fetch(
          `http://127.0.0.1:8000/api/usuarios/${selectedUser.usu_id}/assign-permissions`,
          {
            method: "POST",
            headers: {
              Authorization: `Bearer ${localStorage.getItem("auth_token")}`,
        ...
        if (!response.ok) throw new Error("Error al modificar permiso");
        ...
        showMessage(
          "error",
          error.message || "Error al modificar permiso del usuario"
        );
      } finally {
        setSavingPermissions(false);
      }
    },
    [selectedUser, loadUserPermissions, showMessage]
  );

  // ===== UTILIDADES MEMOIZADAS =====
  const isPermissionAvailableInProfile = useCallback(
    (menId, subId = null, opcId = null) => {
  ...
  const userPermissionsStats = useMemo(() => {
    if (!profilePermissions.length || !selectedUser)
      return { available: 0, granted: 0, percentage: 0 };
          ...
          if (submenu.opciones) {
            submenu.opciones.forEach((opcion) => {
              if (opcion.has_permission) {
                available++;
                if (
                  hasUserPermission(menu.men_id, submenu.sub_id, opcion.opc_id)
                )
                  granted++;
              }
            });
        ...
        {selectedUser ? (
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="flex items-center mt-2 text-sm text-gray-600">
                    ...
                    <span>
                      Permisos asignados: {userPermissionsStats.granted}/
                      {userPermissionsStats.available}
                    </span>
            ...
            <div className="flex-1 overflow-auto">
              {loadingPermissions ? (
                ...
                <div className="text-center py-12 text-gray-500">
                  ...
                  <p className="text-sm mt-1">
                    El perfil del usuario no tiene permisos asignados
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="flex items-center justify-center h-full text-gray-500">
            <div className="text-center">
              ...
              <p className="text-sm mt-1">
                Los usuarios solo pueden tener permisos disponibles en su perfil
              </p>
            </div>
          </div>
        )}
